...
describe("a country standard's own criteria", () => {
  ...
  it("indexes the standard, with the counts an auditor plans against", async () => {
    ...
    // 58 of 106 RGAA criteria declare that NO engine rule can evidence them. Surfacing that
    // is the difference between a plan and a false sense of coverage.
    expect(r.counts.noEngineRule).toBe(58);
