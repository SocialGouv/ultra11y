...
describe("the JSON-RPC contract", () => {
  it("initializes and identifies itself, issuing no session id", async () => {
    ...
    expect(body.result.serverInfo.name).toBe("ultra11y");
  });
  ...
  it("lists tools, and answers a batch with an array", async () => {
    ...
    expect(single.json().result.tools.map((t: { name: string }) => t.name)).toContain("ultra11y_criteria");
  ...
  it("runs a real tool call end to end", async () => {
    const res = await post({ body: { jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "ultra11y_criteria", arguments: { sc: "1.1.1" } } } });
    expect(res.status).toBe(200);
    expect(JSON.parse(res.json().result.content[0].text).sc).toBe("1.1.1");
  });
  ...
  it("returns a failing tool as 200 + isError, never as an HTTP 5xx", async () => {
    ...
    const res = await post({
      body: { jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "ultra11y_audit", arguments: { cwd: "/nope/not/here" } } },
    });
    expect(res.status).toBe(200);
    const result = res.json().result;
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toMatch(/project root not found/);
  });
});
...
describe("statelessness under concurrency", () => {
  it("keeps two overlapping requests on different protocol versions from reading each other's", async () => {
    ...
    const pick = (r: { json: () => { result: { tools: { name: string }[] } } }) =>
      r.json().result.tools.find((t) => t.name === "ultra11y_read") as Record<string, unknown>;
...
describe("protocol version negotiation over HTTP", () => {
  it("accepts a supported version header", async () => {
    ...
    const read = res.json().result.tools.find((t: { name: string }) => t.name === "ultra11y_read");
