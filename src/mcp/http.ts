...
// Every ultra11y tool call is self-contained — it carries its own `repo` and
...
export function startHttpServer(opts: HttpOptions = {}): Promise<RunningHttpServer> {
  ...
  if (!LOOPBACK_BIND.has(bind) && !opts.allowRemote) {
    return Promise.reject(
      new Error(
        `refusing to bind ${bind}: ultra11y's MCP server clones arbitrary git URLs and reads local files. Pass --allow-remote if that is really what you want.`,
      ),
    );
  }
  ...
  // A cold `ultra11y_doc` runs for minutes. Node's default 300s request timeout
